  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="registerList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
			<el-button v-if="scope.row.visitState==1"
			  size="mini"
			  type="text"
...
<script>
import { listRegister, getRegister, delRegister, addRegister, updateRegister,updateStatus } from "@/api/register/register";
...
export default {
  ...
  methods: {
	...
	updateStatus(row){
		const id = row.id;
		this.$modal.confirm('是否确认退号？').then(function() {
	...
	jiezhen(row){
		const id = row.id;
		this.$modal.confirm('是否确认接诊？').then(function() {
